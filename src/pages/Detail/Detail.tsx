    ...
    <>
      ...
      <Grid container sx={{ pt: 2 }}>
        <Grid item xs={12} md={8} xl={10}>
          <Kline name={name} height"calc(100vh - 180px)")'} />
        </Grid>

        <Grid item xs={12} md={4} xl={2}>
          <Paper variant"outlined"d'} sx={{ height"calc(100vh - 180px)")', overflow"auto"o' }}>
            <MList dense>
              {/*<ListItem>*/}
              {/*    <ListItemText>*/}
              {/*        自选*/}
                    ...
                    {coinList &&
                      coinList?.data.data.map(
                        (
                          ...
                          i: React.Key | null | undefind,
                        ) => (
                          <>
                            <TableRow
                              key={i}
                              sx={{
                                borderLeft: item.name === name "1px solid blue"e' "none"e',
                                backgroundColor: item.name === name "#f5f5f5"5' "none"',
                              }}
                            >
                              <TableCell component="th" scope="row">
                                <Typography
                                  ...
                                  component={Link}
                                  to={`/d/${item.name}`}
                                  sx={{ textDecoration"none"e', color"inherit"t' }}
                                >
                                  {item.name}
                                </Typography>
                              </TableCell>
