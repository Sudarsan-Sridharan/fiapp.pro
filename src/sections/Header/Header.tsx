  ...
  return (
    <>
      ...
      {whaleWarningData && swiper == 1 && whaleWarningData.data.length > 0 && (
        <Box
          sx={{ bgcolor: '#000', color: '#fff', textAlign: 'center', cursor: 'pointer' }}
          onClick={() => nav(`/d/${whaleWarningData.data[0].name}`)}
        >
          <Typography variant={'body1'}>
            {timejs(new Date(whaleWarningData.data[0].open_time).toLocaleString()).fromNow()} -{' '}
            {whaleWarningData.data[0].name === 'BTCUSDSHORTS' ? '空头' : '多头'} - 持仓量{' '}
            {whaleWarningData.data[0].value > 0 ? '增加' : '减少'}{' '}
            {(whaleWarningData.data[0].value * 100).toFixed(2)}%
          </Typography>
        </Box>
      )}
    </>
              ...
              <PlainLink sx={{ display: 'block' }}>
                <ListItemText>
                  <Box sx={{ display: 'flex', alignItems: 'center' }}>
                    <img
                      src={'/black-48dp/1x/outline_currency_bitcoin_black_48dp.png'}
                      style={{
